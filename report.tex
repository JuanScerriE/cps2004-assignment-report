...
\ul{Limitation:} The user interface is limited in the amount of
information it can neatly provide to the player, making it
clunky.

\ul{Solution:} Using a GUI would help provide a better UX since
a GUI is very flexible.

\ul{Limitation:} The amount of control the player has, is also
limited and giving the user more granular control would require
more clutter or more nesting in sub--menus. 

\ul{Solution:} Same as the previous limitation.
...
the unit tests fail on Linux. This is due to differing
...
\ul{Solution:} This seems to be intended behaviour for
